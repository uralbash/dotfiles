...
__powerline() {
    if [[ -v URALBASH_POWERLINE ]]
    then
        echo "Uralbash powerline nitializated!" > /dev/null
    else
        readonly URALBASH_POWERLINE=1
    ...
    ps1() {
        ...
        # PS1="$(__vim_mode_info)"
        PS1="$BG_BASE01$FG_BASE3 \w $RESET"
        PS1+="$BG_BLUE$FG_BASE3$(__git_info)$RESET"
        PS1+="$BG_MAGENTA$(__venv_info)$RESET"
        PS1+="$BG_VIOLET$(__nix_info)$RESET"
        PS1+="$BG_YELLOW$(__foreground_info)$RESET\n"
